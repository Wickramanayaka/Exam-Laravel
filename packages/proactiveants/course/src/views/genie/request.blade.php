...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-12">
            @php
                $merchantPgIdentifier = config('course.merchantPgIdentifier');
                $currency = config('course.currency');
                $paymentMethod = config('course.paymentMethod');
                $secretCode = config('course.secretCode');
                $storeName = config('course.storeName');
                $url = config('course.url');
                $transactionType = 'SALE';
                $language = 'en';
                $successUrl = config('course.successUrl');
                $errorUrl = config('course.errorUrl');
                $now = date('Y-m-d H:i:s');
                $amount = $payment->amount;
                $id = $payment->id;
                $invoice = config('course.prefix').$id;
                $token = $storeName.$currency.$secretCode.$now.$amount.$invoice;
                $token = hash('sha256', $token);
            @endphp
            <h3>Pay by Genie</h3>
            <img src="{{asset('images/Genie_Final_LOGO_EPS.JPG')}}" alt="Genie logo" width="125">
            <p>Please wait...</p>
            <hr>
            <form style="display: none" id="ext-merchant-frm" name="ext-merchant-frm" action="{{$url}}" method="post" accept-charset="UTF-8"
                enctype="application/x-www-form-urlencoded">
                <input type="hidden" id="merchantPgIdentifier" name="merchantPgIdentifier" value="{{$merchantPgIdentifier}}">Total Price
                <input type="text" id="chargeTotal" name="chargeTotal" value="{{$amount}}" readonly>
                ...
                <br>
                <input type="submit" value="Proccees to Genie">
            </form>
        </div>
    </div>
</div>
